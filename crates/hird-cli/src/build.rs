...
pub(crate) fn build(
    ...
    if let Some(entry) = &entry {
        ...
        erl_files.push(write_erl(
            out_dir,
            BOOT_MODULE,
            &boot_module(entry, &tool_modules),
        )?);
    }
...
/// Renders the boot module: starts the audit sink (stdout), registers each
...
/// `erl` without the CLI.
fn boot_module(entry: &EntryPoint, tool_modules: &[String]) -> String {
    ...
    let _ = writeln!(
        out,
        "    {{ok, _}} = hird_audit:start_link([{{sink, stdout}}]),"
    );
